...
def pretrained_resnet(
    ...
    if state_dict is None:
        if not torch_exists:
            raise ImportError('Install `torch` and `torchvision` to use this function.')

        # --- FIXED BLOCK START ---
        from torchvision import models
        
        # Determine the model function and weights enum
        model_fn = getattr(models, f'resnet{size}')
        try:
            # Modern Torchvision API (v0.13+)
            weights_name = f'ResNet{size}_Weights'
            weights_enum = getattr(models, weights_name)
            # Use the default ImageNet weights
            pt_model = model_fn(weights=weights_enum.IMAGENET1K_V1)
