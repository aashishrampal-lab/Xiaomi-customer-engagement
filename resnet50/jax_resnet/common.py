...
class ConvBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, train: bool = False): # 1. Added train argument
        x = self.conv_cls(
            self.n_filters,
            self.kernel_size,
            self.strides,
        ...
        if self.norm_cls:
            ...
            # 2. Logic updated: If we are not training, use running average
            # This ensures we use the 76.46% quality pretrained weights correctly.
            x = self.norm_cls(
